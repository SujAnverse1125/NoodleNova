...
| Website interactions | [`admin-panel-couriers-transactions.webp`](admin-panel-couriers-transactions.webp), [`admin-panel-feedback.webp`](admin-panel-feedback.webp), authorized admin panel, and [`interaction-evidence.md`](interaction-evidence.md) | Read-only captures show the registered-user list, 40-entry transaction log, and 69 feedback records; full hashes and the 10-interaction claim remain pending verification. Do not publish the admin token or private admin URL. |

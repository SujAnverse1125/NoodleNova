...
| Product stability | **Implemented locally, hosted CI pending** | `npm test` and `npm run build` pass locally; scoped tests avoid broken tracked symlink discovery. The post-push GitHub Actions run must be checked. |
